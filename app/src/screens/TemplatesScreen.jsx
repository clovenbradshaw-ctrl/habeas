...
export default function TemplatesScreen() {
  ...
  function handleImportFile(e) {
    ...
    if (!['docx', 'doc'].includes(ext || '')) {
      showToast('Only DOCX template uploads are supported.', true);
  ...
  async function handleConfirmImport() {
    ...
    setImporting(true);
    try {
      const template = await importTemplate(pendingImportFile);
      const id = await createTemplate({
        ...template,
      });
    ...
    <div>
      ...
      {pendingImportFile && (
        <div className="mb-4 bg-white border border-gray-200 rounded-[12px] p-4">
          <div className="text-[0.82rem] font-semibold text-gray-800 mb-2">Import DOCX template</div>
          <p className="text-[0.78rem] text-gray-500">{pendingImportFile.name} will be imported using DOCX-first mode. Use placeholders like {{PETITIONER_NAME}} for variables.</p>
